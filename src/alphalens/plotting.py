...
def plot_information_table(ic_data, return_df=False):
    ...
    ic_summary_table["RkIC Mean"] = ic_data.mean()
    ic_summary_table["RkIC Std."] = ic_data.std()
    ic_summary_table["RkICIR"] = ic_data.mean() / ic_data.std()
    # t_stat, p_value = stats.ttest_1samp(ic_data, 0)
    # ic_summary_table["t-stat(RkIC)"] = t_stat
    # ic_summary_table["p-value(RkIC)"] = p_value
    # ic_summary_table["RkIC Skew"] = stats.skew(ic_data)
    # ic_summary_table["RkIC Kurtosis"] = stats.kurtosis(ic_data)
